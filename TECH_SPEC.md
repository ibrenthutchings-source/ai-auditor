...
Audit-log integrity: Because this is a compliance/governance product, its own audit trail must be append-only. `AuditFinding` and `AuditState` records are written once and never updated in place; corrections are new rows referencing the original by `audit_id`. Evaluate a hash-chain (each row includes a hash of the previous row) if tamper-evidence becomes a requirement — not required for MVP, but the append-only table design is. **Done**: `backend/app/models/db_models.py::AuditRecord` (SQLAlchemy, `audit_records` table) — no update/delete path exists for this table by design, only inserts, via `backend/app/core/audit_store.py::persist_audit_record`. Writes are best-effort and non-fatal: a database outage logs a warning and degrades to "this audit wasn't persisted" rather than failing the `/audit` request. `DATABASE_URL` still defaults to a local Postgres for dev; **no Postgres has been provisioned on Railway yet** — the deployed backend currently runs with persistence silently disabled until a real database is provisioned there.
